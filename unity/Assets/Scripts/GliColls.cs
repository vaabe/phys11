...
public class GliColls : MonoBehaviour
{
	...
	public GameObject[] Whl = new GameObject[4]; 
	public GameObject[] Spr = new GameObject[1];  

	private string systemName = "GliColls"; 
	...
	void Start()
	{
		for (int i = 0; i < N; i++)
		{
			...
			particle[i].currPos = Body[i].transform.position; 
...
//		}
		...
		// extras
		Whl[0].transform.Rotate(8, 0, 0, Space.Self); 
		Whl[1].transform.Rotate(8, 0, 0, Space.Self); 
		Whl[2].transform.Rotate(8, 0, 0, Space.Self); 
		Whl[3].transform.Rotate(8, 0, 0, Space.Self); 
		Spr[0].transform.Translate(0, -0.03f, 0, Space.Self); 
	}
}
